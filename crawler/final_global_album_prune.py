...
This pass intentionally ignores artist ownership. It removes a QQ candidate when either:
1) its normalized album title strongly matches any existing BarScope album title; or
2) its track count matches and the tracklist strongly describes the same release.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="忽略艺人归属，按全库专辑名 + tracks 最终压缩 QQ 候选")
