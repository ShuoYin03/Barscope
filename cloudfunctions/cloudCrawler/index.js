...
async function runAlbum(albumId) {
  ...
  await doneStatus(1, result.inserted, `专辑《${raw.name || ''}》新增 ${result.inserted} 张，补全日期 ${result.dated || 0} 张`)
...
async function runArtist(artistId) {
  ...
  await doneStatus(1, result.inserted, `艺人 ${name || artistId}：新增 ${result.inserted} 张，补全日期 ${result.dated || 0} 张`)
...
async function runAllApproved(cursor) {
  ...
  if (cursor === 0) {
    ...
    await appendLog(`云端全量开始：${total} 位已批准艺人；新专辑会同时写入发行日期`)
  }
  ...
  for (let i = 0; i < slice.length; i += 1) {
    ...
    try {
      ...
      await appendLog(`[${cursor + i + 1}/${total}] ${artist.artistName}: 新增${result.inserted}张，日期写入${result.dated}张`)
    } catch (e) { await appendLog(`[${cursor + i + 1}/${total}] ${artist.artistName} 失败: ${e.message}`) }
  }
  const processed = Math.min(cursor + CHUNK, total)
  const status = await getStatus()
  const old = Number((status.progress || {}).albumsFound || 0)
  await patchStatus({ progress: { totalArtists: total, processedArtists: processed, albumsFound: old + added, candidatesFound: 0 } })
  if (processed < total) {
    ...
    return { success: true, status: 'running', processed, total, inserted: added, dated }
  }
  await patchStatus({ status: 'done', completedAt: db.serverDate(), lastRunSummary: { newAlbums: old + added, newCandidates: 0, errors: [] } })
  await appendLog(`云端全量完成：新增 ${old + added} 张，日期写入 ${dated} 张`)
  return { success: true, status: 'done', total, newAlbums: old + added, dated }
}
...
async function fetchArtistAlbums(artistId) {
  ...
  while (true) {
    let data
    try { data = await httpsGet(`https://music.163.com/api/artist/albums/${artistId}?limit=50&offset=${offset}`) } catch (e) { break }
...
async function fetchAlbumById(id) {
  const data = await httpsGet(`https://music.163.com/api/v1/album/${id}`)
  return data && data.code === 200 ? data.album : null
}

function releaseDateFromTime(value) {
...
function normalizeAlbum(raw, fallbackArtist, opts) {
  ...
  const primaryArtist = String((raw.artist || {}).name || fallbackArtist || '').trim()
  const artists = (raw.artists || []).map(x => String(x.name || '').trim()).filter(Boolean)
  const artist = artists.length > 1 ? artists.join(' / ') : primaryArtist
...
async function upsertAlbums(rawList, fallbackArtist, opts) {
  ...
  if (!albums.length) return { inserted: 0, total: 0, dated: 0 }
  const ids = albums.map(x => x.sourceId)
  const existing = new Map()
  for (let i = 0; i < ids.length; i += 100) {
    const res = await db.collection('albums').where({ sourceId: _.in(ids.slice(i, i + 100)) }).field({ _id: true, sourceId: true, releaseDate: true, releaseYear: true, neteaseArtistId: true, primaryArtist: true, trackCount: true }).get()
    ;(res.data || []).forEach(x => existing.set(x.sourceId, x))
  }
  let inserted = 0
  let dated = 0
  await Promise.all(albums.map(async album => {
    ...
    if (!old) {
      await db.collection('albums').add({ data: Object.assign({ approved: true }, album) })
      inserted += 1
      if (album.releaseDate) dated += 1
      return
    }
    const patch = {}
    if (!old.releaseDate && album.releaseDate) { patch.releaseDate = album.releaseDate; patch.releaseYear = album.releaseYear; dated += 1 }
  ...
  return { inserted, total: albums.length, dated }
}
