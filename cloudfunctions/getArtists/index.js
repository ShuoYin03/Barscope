  ...
  try {
    ...
    const [res, countRes] = await Promise.all([
      db.collection('artist_candidates')
        .where(conditions)
        .field({ _id:true, artistId:true, artistName:true, picUrl:true, avatarUrl:true, coverUrl:true, fansSize:true, albumSize:true })
        .limit(limit)
        .get(),
...
function firstLetter(name) {
  for (const ch of Array.from(String(name || '').trim())) {
    if (/[A-Za-z]/.test(ch)) return ch.toUpperCase()
    if (/[\u4e00-\u9fff]/.test(ch)) return pinyinInitial(ch)
  }
  return '#'
}
function pinyinInitial(ch) {
  let letter = '#'
  for (const [initial, startChar] of PINYIN_STARTS) {
    if (ch.localeCompare(startChar, 'zh-Hans-CN-u-co-pinyin') >= 0) letter = initial
    else break
  }
  return letter
}
