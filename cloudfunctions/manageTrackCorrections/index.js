...
exports.main=async event=>{
  try{
    ...
    if(action==='list') return await list(event)
    if(action==='approve') return await approve(event,OPENID)
    if(action==='reject') return await reject(event,OPENID)
    if(action==='stats') return await stats()
    return {success:false,error:'unknown action'}
  }catch(e){
    console.error('manageTrackCorrections failed',e)
    return {success:false,error:formatError(e)}
  }
}

function formatError(e){
  const code=String(e&&((e.errCode!==undefined&&e.errCode)||e.code)||'').trim()
  const message=String(e&&((e.errMsg)||e.message)||e||'未知错误').trim()
  return [code,message].filter(Boolean).join(' · ').slice(0,180)
}

function isCollectionMissingError(e){
  const text=formatError(e)
  return /DATABASE_COLLECTION_NOT_EXIST|-502005|collection.*not exist|集合.*不存在/i.test(text)
}
...
async function ensureCollection(){
  try{
    ...
    try{
      ...
      const text=formatError(createErr)
      if(!/already exist|已存在|-501001/i.test(text))throw createErr
    }
  }
}
...
async function stats(){
  ...
  const r=await db.collection(COLLECTION).where({status:'pending'}).count()
...
async function submit(event,openId){
  ...
  await ensureCollection()
  const pending=await db.collection(COLLECTION).where({albumId,submitterOpenId:openId,status:'pending'}).limit(1).get()
  if(pending.data.length)return {success:false,error:'该专辑已有待审核申请'}
  const clean=tracks.map((t,i)=>({
    songId:String(t.songId||''),
    no:i+1,
    name:String(t.name||'').trim(),
    guests:(Array.isArray(t.guests)?t.guests:[]).map(g=>({id:Number(g.id||0),name:String(g.name||'').trim()})).filter(g=>g.name),
  }))
...
async function list(event){
  ...
  const res=await db.collection(COLLECTION).where({status}).orderBy('createdAt','desc').limit(100).get()
...
async function approve(event,adminOpenId){
  ...
  const tracks=doc.tracks.map((t,idx)=>{
    const old=existingBySongId.get(String(t.songId||''))||{}
    const guests=(t.guests||[]).map(g=>({id:Number(g.id||0),name:String(g.name||'').trim()})).filter(g=>g.name)
    const seen=new Set()
    const artists=[]
    ownerArtists.concat(guests).forEach(a=>{
      const k=a.id?'id:'+a.id:'name:'+a.name
      if(!seen.has(k)){seen.add(k);artists.push(a)}
    })
    return {
      songId:String(t.songId||''),
      no:idx+1,
  ...
  await db.collection(COLLECTION).doc(id).update({data:{status:'approved',reviewedAt:db.serverDate(),reviewedBy:adminOpenId,updatedAt:db.serverDate()}})
  return {success:true}
}
