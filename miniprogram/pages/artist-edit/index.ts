...
Page({
  ...
  onLoad(options){
    ...
    this.setData({statusBarHeight:app.globalData.statusBarHeight,topbarHeight:app.globalData.topbarHeight,artistId})
    if(!artistId){wx.showToast({title:'缺少艺人ID',icon:'none'});return}
    wx.cloud.callFunction({name:'getArtist',data:{artistId},success:(res:any)=>{
      ...
      this.setData({
        ...
        avatarUrl:a.avatarUrl||a.picUrl||'',
        heroImageUrl:a.heroImageUrl||a.backgroundUrl||a.coverUrl||'',
        briefDesc:a.briefDesc||a.description||'',
        roles,
        roleOptions:ROLE_OPTIONS.map(x=>({...x,selected:roles.includes(x.key)})),
        loading:false,
      })
  ...
  _chooseImage(kind:'avatar'|'hero'){
    ...
    wx.chooseMedia({count:1,mediaType:['image'],sourceType:['album','camera'],success:(res:any)=>{
      ...
      wx.cloud.uploadFile({cloudPath:`artist-images/manual/${this.data.artistId}_${kind}_${Date.now()}.${ext}`,filePath,success:(up:any)=>{
        wx.hideLoading()
        this.setData({[kind==='avatar'?'avatarUrl':'heroImageUrl']:up.fileID,[loadingKey]:false} as any)
      },fail:()=>{wx.hideLoading();this.setData({[loadingKey]:false} as any);wx.showToast({title:'图片上传失败',icon:'none'})}})
    }})
  },
  ...
  onSave(){
    ...
    wx.cloud.callFunction({name:'manageArtistCorrections',data:{action:'submit',artistId:this.data.artistId,artistName,roles:this.data.roles,avatarUrl:this.data.avatarUrl,heroImageUrl:this.data.heroImageUrl,briefDesc:String(this.data.briefDesc||'').trim()},success:(res:any)=>{
      wx.hideLoading();this.setData({saving:false});const r=res.result||{}
      if(!r.success){wx.showToast({title:r.error||'提交失败',icon:'none'});return}
      wx.showToast({title:'已提交管理员审核',icon:'success'});setTimeout(()=>wx.navigateBack(),700)
    },fail:(err:any)=>{wx.hideLoading();this.setData({saving:false});wx.showToast({title:String(err&&err.errMsg||'提交失败').slice(0,28),icon:'none'})}} as any)
  },
})