...
const COL = 'mixtape5_2026_ballots'
const MAX_ENTRIES = 5
...
async function submit(rawEntries) {
  const { OPENID } = cloud.getWXContext()
  if (!OPENID) return { success: false, error: '请先登录' }
  if (!isVotingOpen()) return { success: false, error: '2026 五大 Mixtape 投票已截止' }
  ...
  for (const raw of list) {
    ...
    if (album.releaseType !== 'Mixtape') return { success: false, error: `《${album.title || albumId}》不是 Mixtape 类型` }
    entries.push({
      albumId,
