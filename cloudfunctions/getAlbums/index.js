...
function baseFilters(genre,year,month){const filters={approved:true};if(genre)filters.genres=_.all([genre]);return applyPeriodFilters(filters,year,month)}
function ratedFilters(genre,year,month){return Object.assign(baseFilters(genre,year,month),{avgScore:_.gt(0),reviewCount:_.gt(0)})}
function unratedFilters(genre,year,month){return Object.assign(baseFilters(genre,year,month),{avgScore:_.lte(0)})}
...
async function searchAlbums({keyword,genre,year,month,page,pageSize}){
  ...
  filtered=filtered.filter(a=>!genre||(a.genres||[]).includes(genre)).filter(a=>{if(!year||isAllYear(year))return true;const y=a.releaseYear;return year==='2010s'?y>=2010&&y<=2017:year==='2000s'?y>=2000&&y<=2009:y===parseInt(year)}).filter(a=>!month||!year||!/^\d{4}$/.test(String(year))||String(a.releaseDate||'').slice(5,7)===String(month).padStart(2,'0'))
