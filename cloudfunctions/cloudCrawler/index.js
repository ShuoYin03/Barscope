...
async function runAllApproved(cursor) {
  ...
  for (let i = 0; i < slice.length; i++) {
    ...
    try {
      ...
      const count = await db.collection('albums').where({ collaboratorArtistIds: _.all([String(artist.artistId)]) }).count()
...
function normalizeAlbum(raw, fallback, opts) {
  opts = opts || {}
  const title = String(raw.name || '').trim()
  const primaryArtist = String((raw.artist || {}).name || fallback || '').trim()
  const rawArtists = raw.artists || (raw.artist ? [raw.artist] : [])
  const collaboratorArtists = rawArtists.map(a => ({ id: a && a.id ? String(a.id) : '', name: String(a && a.name || '').trim() })).filter(a => a.id || a.name)
  ...
  const neteaseArtistId = raw.artist && raw.artist.id ? String(raw.artist.id) : ''
  ...
  const artist = collaboratorArtistNames.length > 1 ? collaboratorArtistNames.join(' / ') : primaryArtist
  const sourceId = String(raw.id || '')
  const coverUrl = raw.picUrl || raw.blurPicUrl || ''
  const releaseDate = releaseDateFromTime(raw.publishTime)
  ...
  if (!opts.skipFilters) {
    ...
    if (releaseYear < 1990 || releaseYear > now + 1 || trackCount < 3 || SKIP_KEYWORDS.some(k => title.includes(k))) return null
  }
  return { title, artist, primaryArtist, neteaseArtistId, collaboratorArtists, collaboratorArtistIds, collaboratorArtistNames, sourceId, coverUrl, releaseYear, releaseDate, genres: [], source: 'netease', crawlSource: 'cloud', avgScore: 0, reviewCount: 0, trackCount }
}
function normalizeTrackName(n) { return String(n || '').replace(/[（(【\[][^）)】\]]*[）)】\]]/g, '').replace(/(伴奏|instrumental|inst\.?|off\s*vocal|karaoke|纯音乐|伴奏版|伴奏带)/ig, '').replace(/[\s\-_.·]/g, '').toLowerCase() }
function inspectAlbumTracks(songs) { const names = (songs || []).map(s => String(s.name || '').trim()).filter(Boolean), accompaniment = names.filter(n => /(伴奏|instrumental|inst\.?|off\s*vocal|karaoke|纯音乐|伴奏版|伴奏带)/i.test(n)), norm = names.map(normalizeTrackName).filter(Boolean), allSame = norm.length >= 2 && new Set(norm).size === 1; if (accompaniment.length) return { bad: true, reason: '含有伴奏/纯音乐版本曲目', example: accompaniment.slice(0, 4) }; if (allSame) return { bad: true, reason: '全专曲目名称重复', example: names.slice(0, 4) }; return { bad: false } }
...
async function mapWithConcurrency(items, limit, fn) { let cursor = 0; await Promise.all(Array.from({ length: Math.min(limit, items.length) }, async () => { while (true) { const i = cursor++; if (i >= items.length) return; await fn(items[i]) } })) }

async function upsertAlbums(rawList, fallback, opts) {
  const albums = rawList.map(x => normalizeAlbum(x, fallback, opts)).filter(Boolean)
  if (!albums.length) return { inserted: 0, total: 0, dated: 0, candidates: 0 }
  const ids = albums.map(x => x.sourceId), existing = new Map()
  for (let i = 0; i < ids.length; i += 100) {
    const r = await db.collection('albums').where({ sourceId: _.in(ids.slice(i, i + 100)) }).field({ _id: true, sourceId: true, releaseDate: true, releaseYear: true, neteaseArtistId: true, primaryArtist: true, trackCount: true, collaboratorArtists: true, collaboratorArtistIds: true, collaboratorArtistNames: true }).get()
    ;(r.data || []).forEach(x => existing.set(x.sourceId, x))
  }
  ...
  await mapWithConcurrency(albums, DETAIL_CONCURRENCY, async album => {
    ...
    if (old) {
      const patch = {}
      if (!old.releaseDate && album.releaseDate) { patch.releaseDate = album.releaseDate; patch.releaseYear = album.releaseYear; dated++ }
      ...
      if (!old.trackCount && album.trackCount) patch.trackCount = album.trackCount
      const oldIds = Array.isArray(old.collaboratorArtistIds) ? old.collaboratorArtistIds : []
      if (album.collaboratorArtistIds.length && JSON.stringify(oldIds) !== JSON.stringify(album.collaboratorArtistIds)) patch.collaboratorArtistIds = album.collaboratorArtistIds
