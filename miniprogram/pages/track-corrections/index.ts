...
Page({
 ...
 approve(e:WechatMiniprogram.TouchEvent){const id=String((e.currentTarget.dataset as any).id||'');if(!id||this.data.operating)return;wx.showModal({title:'通过曲目修改？',content:'通过后将立即更新正式专辑曲目。',confirmText:'通过',confirmColor:'#2D6FE0',success:m=>{if(!m.confirm)return;this.run('approve',id)}})},
