...
Page({
  data:{
    ...
    mode:'search' as 'search'|'manual',searchName:'',searching:false,submitting:false,
  ...
  onSubmitManual(){
    ...
    const tracks=this.data.trackRows
      .map(r=>({name:r.name.trim(),guests:r.guests.map(g=>({id:g.artistId,name:g.artistName}))}))
      .filter(t=>t.name)
