...
Page({
  ...
  onShow() { if (typeof this.getTabBar === 'function') this.getTabBar()?.setData({ selected: 4 }); const app = getApp<IAppOption>(); const loggedIn = !!app.globalData.userInfo; this.setData({ isLoggedIn: loggedIn, userInfo: app.globalData.userInfo, isCritic: app.globalData.userType === 'critic' }); if (loggedIn) this._loadReviews() },

  _loadReviews() {
    this.setData({ loading: true })
    wx.cloud.callFunction({ name:'getUserInfo', success:(res:any)=>{
      ...
      wx.cloud.callFunction({name:'getReviews',data:{userId:user.openId,pageSize:20},success:(r:any)=>{const rv=r.result;const list=(rv.list||[]).map((item:any)=>({_id:item._id,albumId:item.albumId,albumTitle:item.albumTitle||item.albumId,ratingText:'★'.repeat(item.rating||0),content:item.content,timeAgo:item.timeAgo||'',likes:item.likes||0}));this.setData({reviews:list,albumCount:new Set(list.map((x:ProfileReview)=>x.albumId)).size,loading:false})},fail:()=>this.setData({loading:false})})
  ...
  _loadPendingCount() {
    ...
    wx.cloud.callFunction({ name:'manageAlbumOwnershipCorrections',data:{action:'stats'},success:(res:any)=>{const r=res.result;if(r.success)this.setData({ownershipCorrectionCount:r.pending || 0})} })
  },
  onAdminCandidates() { wx.navigateTo({ url:'/pages/admin/index' }) },
  onAdminAlbumFlags() { wx.navigateTo({ url:'/pages/album-candidates/index' }) },
  onAdminOwnershipCorrections() { wx.navigateTo({ url:'/pages/ownership-corrections/index' }) },
  onAdminCrawlerReports() { wx.navigateTo({ url:'/pages/crawler-reports/index' }) },
  ...
  onSubmitArtistRequest() {
    wx.showModal({ title:'提交 rapper 申请', editable:true, placeholderText:'输入网易云 rapper 名称', confirmText:'提交审核', success:(modal:any)=>{
      ...
      wx.cloud.callFunction({ name:'submitArtistRequest', data:{ name }, success:(res:any)=>{ wx.hideLoading(); const r=res.result || {}; if (!r.success) { wx.showToast({ title:r.error || '提交失败', icon:'none' }); return } if (r.existed) { const text=r.status === 'approved' ? '该 rapper 已收录' : r.status === 'pending' ? '该 rapper 已在审核中' : '该 rapper 已有审核记录'; wx.showToast({ title:text, icon:'none' }); return } wx.showToast({ title:`已提交 ${r.artistName}`, icon:'success' }) }, fail:()=>{wx.hideLoading();wx.showToast({title:'提交失败',icon:'none'})} })
    } })
  },
})