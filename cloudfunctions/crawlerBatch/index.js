...
exports.main = async (event) => {
  ...
  const approved = await db.collection('artist_candidates')
    .where({ status: 'approved' })
    .field({ _id: true, artistId: true, artistName: true })
    .limit(1000)
    .get()
  ...
  if (status.abort) {
    await markAborted(status, total)
    return { success: true, status: 'aborted', nextCursor: null }
  }
  ...
  for (const artist of slice) {
    ...
    if (latest.abort) {
      await markAborted(latest, total)
      return { success: true, status: 'aborted', nextCursor: null }
    }

    let result = { inserted: 0, candidates: 0 }
    let err = ''
    try { result = await syncArtist(String(artist.artistId), artist.artistName || '') } catch (e) { err = e.message }

    const current = await getStatus()
    const p = current.progress || {}
    const processed = Number(p.processedArtists || cursor) + 1
    current.status = 'running'
    current.progress = {
      totalArtists: total,
      processedArtists: processed,
      albumsFound: Number(p.albumsFound || 0) + result.inserted,
      candidatesFound: Number(p.candidatesFound || 0) + result.candidates,
    }
    current.log = prependLog(current.log, err
      ? `[${processed}/${total}] ${artist.artistName || artist.artistId} 失败：${err}`
      : `[${processed}/${total}] ${artist.artistName || artist.artistId}：新增${result.inserted}张，候选${result.candidates}张`)
    await saveStatus(current)
  }

  const after = await getStatus()
  if (after.abort) {
    await markAborted(after, total)
    return { success: true, status: 'aborted', nextCursor: null }
  }
  ...
  if (nextCursor >= total) {
    ...
    after.status = 'done'
    after.abort = false
    after.completedAt = db.serverDate()
    after.lastRunSummary = {
      newAlbums: Number(p.albumsFound || 0),
      newCandidates: Number(p.candidatesFound || 0),
      errors: [],
    }
  ...
  for (const raw of rawAlbums) {
    ...
    if (isLowQuality(detail && detail.songs)) {
      ...
      if (!candidate.data.length) {
        await db.collection('album_candidates').add({ data: { ...album, approved: false, status: 'pending', crawlSource: 'cloud-initial-quality-filter', addedAt: db.serverDate() } })
        candidates += 1
      }
      continue
    }
...
function normalizeAlbum(raw, fallbackName) {
  ...
  const names = rawArtists.map(x => String(x.name || '').trim()).filter(Boolean)
  const ids = [...new Set(rawArtists.map(x => String(x.id || '')).filter(Boolean))]
  const primaryArtist = String((raw.artist || {}).name || fallbackName || names[0] || '').trim()
  const sourceId = String(raw.id || '')
  const releaseDate = dateFromTimestamp(raw.publishTime)
  const releaseYear = releaseDate ? Number(releaseDate.slice(0, 4)) : 0
  const trackCount = Number(raw.size || 0)
  if (!title || !sourceId || !primaryArtist || !releaseDate || trackCount < 3) return null
  if (releaseYear < 1990 || releaseYear > new Date().getFullYear() + 1) return null
  if (SKIP_KEYWORDS.some(k => title.includes(k))) return null
  return { title, artist: names.length > 1 ? names.join(' / ') : primaryArtist, primaryArtist, neteaseArtistId: String((raw.artist || {}).id || ids[0] || ''), collaboratorArtistIds: ids, sourceId, coverUrl: raw.picUrl || raw.blurPicUrl || '', releaseYear, releaseDate, genres: [], source: 'netease', crawlSource: 'cloud-batch', avgScore: 0, reviewCount: 0, trackCount }
}

function isLowQuality(songs) {
  const names = (songs || []).map(x => String(x.name || '').trim()).filter(Boolean)
  if (names.some(n => /(伴奏|instrumental|inst\.?|off\s*vocal|karaoke|纯音乐|伴奏版|伴奏带)/i.test(n))) return true
  const normalized = names.map(n => n.replace(/[（(【\[][^）)】\]]*[）)】\]]/g, '').replace(/(伴奏|instrumental|inst\.?|off\s*vocal|karaoke|纯音乐|伴奏版|伴奏带)/ig, '').replace(/[\s\-_.·]/g, '').toLowerCase()).filter(Boolean)
