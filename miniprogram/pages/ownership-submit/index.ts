...
Page({
  ...
  searchArtists(keyword=''){this.setData({artistSearching:true});wx.cloud.callFunction({name:'getArtists',data:{keyword:String(keyword||'').trim(),limit:30},success:(res:any)=>{const r=res.result||{};this.setData({artistResults:r.success?(r.list||[]):[],artistSearching:false})},fail:(e:any)=>{console.error('[getArtists] fail', e);this.setData({artistSearching:false,artistResults:[]})}} as any)},
