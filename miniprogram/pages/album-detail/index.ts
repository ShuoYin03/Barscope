...
Page({
  ...
  onGuestTap(e: WechatMiniprogram.TouchEvent) { const ds = e.currentTarget.dataset as any; if (ds.id && ds.name) wx.navigateTo({ url: '/pages/artist/index?artistId=' + ds.id + '&artistName=' + encodeURIComponent(ds.name) }) },
  ...
  onLike(e: WechatMiniprogram.CustomEvent) { if (!this.data.isLoggedIn) { wx.navigateTo({ url: '/pages/login/index' }); return }; const reviewId = e.detail.reviewId; const review = this.data.reviews.filter((r: ReviewEntry) => r._id === reviewId)[0]; if (!reviewId || !review || review.likedByMe) return; this.setData({ reviews: this.data.reviews.map((r: ReviewEntry) => r._id === reviewId ? { ...r, likes: r.likes + 1, likedByMe: true } : r) }); wx.cloud.callFunction({ name: 'likeReview', data: { reviewId } } as any) },
  onReply(e: WechatMiniprogram.CustomEvent) { if (!this.data.isLoggedIn) { wx.navigateTo({ url: '/pages/login/index' }); return }; const info = e.detail || {}; if (!info.reviewId) return; wx.showModal({ title: '回复 ' + (info.userName || ''), editable: true, placeholderText: '写下你的回复', confirmText: '发送', success: (modal: any) => { const content = String(modal.content || '').trim(); if (!modal.confirm || !content) return; wx.cloud.callFunction({ name: 'replyReview', data: { reviewId: info.reviewId, content } } as any) } }) },
  onFavoriteToggle() { if (!this.data.isLoggedIn) { wx.navigateTo({ url: '/pages/login/index' }); return }; const album = this.data.album; if (!album) return; const old = this.data.isFavorited; this.setData({ isFavorited: !old }); wx.cloud.callFunction({ name: old ? 'removeFavorite' : 'addFavorite', data: { albumId: album.id }, fail: () => this.setData({ isFavorited: old }) } as any) },
})
